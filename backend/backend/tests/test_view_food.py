...
from .data_for_testing import test_food_data_1, test_fridge_item_1, test_user, test_post_fridge_item_1, test_post_fridge_item_2
...
class FoodTestCase(TestCase):
    ...
    def test_manage_fridge_get(self):
        ...
        response = client.put('/api/fridge/item/1/', json.dumps(test_post_fridge_item_2), content_type='applications/json')
        self.assertEqual(response.status_code, 401)
        # get from nonexistent fridgeItem: should get 404
        response = client.get(f'/api/fridge/item/2/')
    ...
    def test_manage_fridge_login(self):
        ...
        response = client.post(f'/api/fridge/{user_id}/user/', json.dumps(test_post_fridge_item_1), content_type='application/json')
        ...
        response = client.post(f'/api/fridge/{user_id}/user/', json.dumps(test_user), content_type='applications/json')
        ...
        self.assertEqual(response.status_code, 200)
        response = client.put(f'/api/fridge/item/{new_id}/', json.dumps(test_post_fridge_item_2), content_type='applications/json')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(json.loads(response.content)['quantity'], 100)
        response = client.put(f'/api/fridge/item/{new_id-1}/', json.dumps(test_post_fridge_item_2), content_type='applications/json')
        self.assertEqual(response.status_code, 403)
        response = client.put(f'/api/fridge/item/{new_id}/', json.dumps(test_user), content_type='applications/json')
        self.assertEqual(response.status_code, 400)

        # You cannot post to this url
        response = client.post(f'/api/fridge/item/{new_id}/', json.dumps(test_post_fridge_item_2), content_type='applications/json')
        self.assertEqual(response.status_code, 405)
